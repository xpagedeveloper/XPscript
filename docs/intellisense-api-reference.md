...
| Member | Syntax | Parameters | Description | Example |
|---|---|---|---|---|
| `CompilerCommandLine.ValidateAsync` | `xpscript validate source.xps [--rid runtime] [--result-format FORMAT] (FORMAT: text, json, or xml)` | source path, optional runtime identifier and result format; `--stdin --filename name.xps` accepts virtual source input. | Validates XPScript through the normal compiler pipeline without producing or executing the final application. | [malformed-source-error.xps](../samples/malformed-source-error.xps) |
