...
| `AttachmentCollection.Update` | `files.Update(attachmentId, sourcePath)` | Existing attachment ID and replacement local file. | Replaces binary content while preserving `attachmentId`, `created` and `createdBy`; updates size/checksum/modified fields. | [database-attachments.xps](../samples/database-attachments.xps) |
...
Call files.SetActor(Session.UserName)
