...
| Property | Type | Access | Description |
| --- | --- | --- | --- |
| `ReturnMessage` | String | read-only | Standard output produced by the most recent `NotesAgent.Run` or `RunOnServer` call. It is reset to an empty string when no output is returned. |

`ReturnMessage` contains the agent output after the run has completed. It does not include compiler or runtime diagnostics generated by XPscript itself.
