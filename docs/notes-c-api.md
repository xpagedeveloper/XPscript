...
The repository includes `samples/notes-c-api-surface.xps`. Its `Main` routine does not initialize Notes, so normal CI can compile the surface without an HCL installation. The helper routine exercises the generated API and is intended to be adapted for manual testing on a system with the HCL Notes/Domino runtime installed.
