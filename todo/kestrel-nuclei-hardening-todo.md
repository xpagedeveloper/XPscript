...
- [x] Verify credential storage and cookie behavior across HTTP and HTTPS. Browser WASM explicitly verifies the shared correlation cookie policy: HttpOnly, SameSite=Lax, 30-day lifetime, Secure on HTTPS and intentionally omitted on HTTP. Native XPHttpClient cookie storage remains disabled; verified in run #257.
