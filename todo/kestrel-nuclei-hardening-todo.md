...
- [~] Add applicable generic fuzzing templates in a controlled phase. The required matrix is ordinary web application plus REST server application across Kestrel, CGI and FastCGI. CI now runs the deliberately bounded upstream Linux LFI, Windows LFI and X-Forwarded-For 403-bypass corpus against separate ordinary-web and REST Kestrel targets with concurrency 1, bulk size 1 and a 20 request/second rate limit. CGI and real nginx-to-FastCGI web/REST targets remain before this item can be completed.
