...
- [~] Add applicable generic fuzzing templates in a controlled phase. The required matrix is ordinary web application plus REST server application across Kestrel, CGI and FastCGI. CI now runs the deliberately bounded upstream Linux LFI, Windows LFI and X-Forwarded-For 403-bypass corpus against separate ordinary-web and REST Kestrel targets with concurrency 1, bulk size 1 and a 20 request/second rate limit. CI now also contains direct CGI web/REST fuzzing and real nginx-to-FastCGI web/REST fuzzing. All six required application/hosting combinations are implemented and await end-to-end CI verification before this item can be completed.
