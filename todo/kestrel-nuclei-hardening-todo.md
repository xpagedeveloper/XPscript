...
- [~] Verify browser routes and server routes enforce the same authorization policy. Browser/WASM shell and assets remain anonymous so login/onboarding UI can load, while `[ServerSide]` procedures now carry independent `[Anonymous]`, `[Authenticated]`, `[Role:...]` and `[Rule:...]` policies enforced by the server bridge; compilation and hardening workflow verified in run #275. Direct authenticated/role/rule bridge invocation regressions remain before completion.
