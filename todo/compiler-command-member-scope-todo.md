...
- [ ] Document the identifier scopes used by the compiler: types, module/global symbols, class members, procedure parameters, locals, enum members, and compiler-generated symbols.
- [ ] Distinguish lexical/syntax keywords from runtime API names and compiler rewrite markers.
- [ ] Define when an unqualified function call resolves to a built-in/runtime function.
- [ ] Define member-access behavior so `object.Name` / `object.Name(...)` is never treated as an unqualified global command solely because `Name` matches a runtime function.
- [ ] Verify case-insensitive resolution consistently.

## 2. Audit reserved identifier validation

- [ ] Audit `ReservedIdentifierPreprocessor` by declaration kind and scope.
- [ ] Keep compiler-generated `__*` protection.
- [ ] Keep runtime type-name protection only where a type declaration would actually collide.
- [ ] Verify runtime value names such as `Application` and `Body` are restricted only in scopes where the collision is real.
- [ ] Ensure ordinary runtime commands/functions are not added to a global reserved-name list.
...
- [ ] Audit `NativeHttpJsonPreprocessor`, especially `JsonParse`, `JsonStringify`, `JsonEncode`, and `JsonDecode`.
- [ ] Audit `ReferenceRuntimeExtensionsPreprocessor`; preserve its existing member-access exclusion behavior.
...
- [ ] Add a class property whose name matches a runtime/global function.
- [ ] Add a class method whose name matches a runtime/global function.
- [ ] Add a parameter/local whose name matches a runtime/global function where syntactically valid.
- [ ] Verify `obj.JsonParse(...)` remains a member call.
- [ ] Verify `JsonParse(...)` still resolves to the native JSON runtime function.
- [ ] Verify `obj.StrLeftBack(...)` remains a member call.
- [ ] Verify `StrLeftBack(...)` still resolves to the reference runtime function.
...
- [ ] Run regressions through the real XPScript transpiler/compiler, not only string-level unit tests.
