...
- [x] Permit `Null` only for Variant-compatible direct assignments and parameters where the source type validator can resolve the target type; typed local and module-global scalar assignments are rejected with bounded XPScript diagnostics. Cross-platform fixtures: `samples/null-integer-assignment-error.xps` and `samples/null-module-global-assignment-error.xps`.
