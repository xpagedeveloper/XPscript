...
- [x] Add applicable generic fuzzing templates in a controlled phase. The required matrix is ordinary web application plus REST server application across Kestrel, CGI and FastCGI. CI runs the deliberately bounded upstream Linux LFI, Windows LFI and X-Forwarded-For 403-bypass corpus against separate ordinary-web and REST Kestrel targets with concurrency 1, bulk size 1 and a 20 request/second rate limit, plus direct CGI web/REST fuzzing and real nginx-to-FastCGI web/REST fuzzing. All six required application/hosting combinations passed end-to-end hardening and Windows IIS CI in run #309.
- [x] Add applicable generic vulnerability templates in a controlled phase. CI runs the pinned upstream cache-poisoning, cache-poisoning-XSS, CORS misconfiguration, Linux LFI, Windows LFI, open-redirect and reflected-URI-XSS generic vulnerability templates with concurrency 1, bulk size 1 and a 20 request/second rate limit. OOB interaction, SQL injection, XXE, J2EE-specific, XML-RPC-specific and broad fuzz templates are intentionally excluded from this controlled phase pending separate applicability review. The controlled generic vulnerability scan, severity-policy enforcement, fuzzing matrices, raw HTTP probes, nginx-to-Kestrel request-smuggling probes and Windows IIS deployment contract all passed end-to-end in run #313.
- [x] Add ASP.NET Core and Kestrel-specific CVE templates when relevant. Reviewed the pinned Nuclei template tree at NUCLEI_TEMPLATES_SHA 883b9d5b0f45857738f35446cdb1b03e09d969d5. It contains the already-enabled ASP.NET Core development-environment and ASP.NET/IIS exposure/detection templates, but no Kestrel-specific or applicable ASP.NET Core CVE HTTP templates to add. Product-specific templates such as DotNetCMS and DotNetNuke are not applicable to XPScript. Re-review is covered by the monthly pinned scanner/template review.
