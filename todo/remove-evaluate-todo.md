...
Branch: `feature/remove-evaluate`
...
- [ ] Identify references that are historical only and must remain untouched.

## 2. Compiler and transpiler

- [ ] Remove parsing or recognition of `Evaluate` from the compiler frontend.
- [ ] Remove Evaluate-specific transpiler branches and code generation.
- [ ] Remove compiler diagnostics that only exist for Evaluate.
- [ ] Remove Evaluate-specific compiler helpers that become unused.
- [ ] Confirm normal expression, function and procedure compilation is unaffected.

## 3. Preprocessor and postprocessor pipeline

- [ ] Remove Evaluate-specific preprocessors.
- [ ] Remove Evaluate-specific source rewriting.
- [ ] Remove Evaluate-specific postprocessing or generated-code cleanup.
- [ ] Remove registration of these processors from the pipeline.
- [ ] Remove dead utility methods and imports left after the cleanup.

## 4. Runtime

- [ ] Remove the Evaluate runtime entry points.
- [ ] Remove the Evaluate parser/interpreter implementation.
- [ ] Remove Evaluate-only runtime models, tokens, AST nodes and helpers.
- [ ] Remove runtime registrations and dispatch cases for Evaluate.
- [ ] Remove public runtime APIs that expose Evaluate.
- [ ] Verify no dynamic execution path remains under another name by accident.

## 5. Samples and fixtures

- [ ] Remove all `samples/evaluate-*` files.
...
- [ ] Remove `docs/evaluate.md` if present.
...
- [ ] Remove obsolete Evaluate TODO or closeout documents once their information is no longer needed.
- [ ] Keep historical Git history unchanged.
...
- [ ] Ensure `main` was not modified directly.
