...
function Invoke-Bounded([string] $fileName, [string[]] $arguments, [int] $timeoutMilliseconds, [string] $label) {
  ...
  try {
    ...
    if (-not $p.WaitForExit($timeoutMilliseconds)) {
      ...
      [void] $p.WaitForExit(10000); exit 124
    }
...
function Compile-Xps([string] $source, [string] $name) {
  ...
  $r = Invoke-Bounded 'dotnet' @($compilerDll,$source,'-o',"./out/fulltest/$name",'--runtime=false') $compileTimeoutMilliseconds "compile $name"
  if ($r.ExitCode -ne 0) { exit $r.ExitCode }
}
function Get-XpsExe([string] $name) {
  $plain = "./out/fulltest/$name"; $win = "$plain.exe"
  if (Test-Path $win -PathType Leaf) { return (Resolve-Path $win).Path }
  if (Test-Path $plain -PathType Leaf) { return (Resolve-Path $plain).Path }
  throw "Executable not found: $name"
}
function Run-Xps([string] $source, [string] $name, [string[]] $arguments = @()) {
  Compile-Xps $source $name
  Write-Host "FULLTEST_RUN=$name"
  $r = Invoke-Bounded (Get-XpsExe $name) $arguments $runtimeTimeoutMilliseconds "run $name"
  if ($r.ExitCode -ne 0) { exit $r.ExitCode }
  return $r
}
function Expect-XpsFailure([string] $name, [string[]] $arguments, [string] $label) {
  $r = Invoke-Bounded (Get-XpsExe $name) $arguments $runtimeTimeoutMilliseconds "security $label"
  if ($r.ExitCode -eq 0) { throw "Security probe unexpectedly succeeded: $label" }
  if ([string]::IsNullOrWhiteSpace($r.Output)) { throw "Security probe returned no diagnostic: $label" }
  if ($r.Output -match 'SharpCompress') { throw "Security diagnostic exposed implementation detail: $label" }
}
...
$r = Invoke-Bounded 'dotnet' @($compilerDll,'./samples/notes-document-send-surface.xps','-o','./out/fulltest/notes-send','--runtime=false') $compileTimeoutMilliseconds 'NotesDocument.Send diagnostics'
if ($r.ExitCode -ne 0) { exit $r.ExitCode }
...
$r = Invoke-Bounded 'dotnet' @('run','--project','./tests/NotesRichTextDiagnostic/NotesRichTextDiagnostic.csproj','-c','Release','--','./samples/notes-database-query-access-runtime-test.xps','./out/fulltest/notes-query-access') $compileTimeoutMilliseconds 'Notes QueryAccess surface'
if ($r.ExitCode -ne 0) { exit $r.ExitCode }
if (-not $IsWindows -and $env:RUNNER_OS -eq 'Linux') {
  $r = Invoke-Bounded 'dotnet' @('run','--project','./tests/NotesFullRuntimeSurfaceAudit/NotesFullRuntimeSurfaceAudit.csproj','-c','Release','--','NotesSession','NotesDocument','NotesDatabase','NotesDatabaseAccess') $compileTimeoutMilliseconds 'Notes surface audit'
  if ($r.ExitCode -ne 0) { exit $r.ExitCode }
}
Write-Host 'NOTES_FULLTEST: passed'

Write-Host '=== XP RUNTIME FULLTEST ==='
$r = Invoke-Bounded 'dotnet' @('run','--project','./tests/SpreadsheetCapabilityProbe/SpreadsheetCapabilityProbe.csproj','-c','Release') $compileTimeoutMilliseconds 'Spreadsheet compiler probes'
if ($r.ExitCode -ne 0) { exit $r.ExitCode }
...
if (-not $IsWindows) {
  ...
  if ($null -eq $xlsx) { throw 'XPSpreadsheet round-trip did not create expected XLSX.' }
  Add-Type -AssemblyName System.IO.Compression
  $zip = [System.IO.Compression.ZipFile]::OpenRead($xlsx.FullName)
  try {
    $marker = $zip.GetEntry('docProps/custom.xml'); if ($null -eq $marker) { throw 'XPSpreadsheet workbook marker is missing.' }
    $reader = [System.IO.StreamReader]::new($marker.Open()); try { $text = $reader.ReadToEnd() } finally { $reader.Dispose() }
    if ($text -notmatch 'XPScriptWorkbookVersion' -or $text -notmatch '>2<') { throw 'XPSpreadsheet workbook marker is invalid.' }
  } finally { $zip.Dispose() }
}
...
if (Test-Path './out/archive-security-fixtures/extraction-symlink-ready.txt') {
  Compile-Xps ./demo/archive/archive-security-reject-extract.xps archive-security-reject-extract
  Expect-XpsFailure archive-security-reject-extract @() 'destination symbolic link extraction'
  if (Test-Path './out/archive-security-fixtures/outside/escape.txt') { throw 'Archive extraction escaped through destination symbolic link' }
}
...
Write-Host 'ALL_FULLTESTS: passed'
