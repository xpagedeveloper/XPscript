...
internal static class XpsHostConfig
{
    ...
    private static List<string> BuildFastCgiArgs(FastCgiConfig? config, string baseDirectory, string[] cliArgs)
    {
        ...
        AddValue(result, cliArgs, "--root", config.Root is null ? null : ResolvePath(baseDirectory, config.Root)); AddValue(result, cliArgs, "--default-document", config.DefaultDocument); AddValue(result, cliArgs, "--listen", config.Listen); AddValue(result, cliArgs, ["--address", "--bind"], "--address", config.Address); AddValue(result, cliArgs, "--port", config.Port); AddValue(result, cliArgs, "--unix-socket", config.UnixSocket is null ? null : ResolvePath(baseDirectory, config.UnixSocket)); AddValue(result, cliArgs, "--log-directory", config.LogDirectory is null ? null : ResolvePath(baseDirectory, config.LogDirectory)); return result;
    }
