...
internal static class XpsOpenApiCommand
{
    ...
    private static int RunClient(string[] args)
    {
        ...
        if (command == "update" && File.Exists(outputPath))
        {
            ...
            if (!IsGeneratorOwnedClient(existing))
    ...
    private static bool IsGeneratorOwnedClient(string source)
    {
        return source.Contains("' <xpscript-openapi-client", StringComparison.OrdinalIgnoreCase)
            || source.Contains("' Generated by XPScript OpenAPI client generator", StringComparison.OrdinalIgnoreCase)
            || source.Contains("' Generated from OpenAPI", StringComparison.OrdinalIgnoreCase);
    }
