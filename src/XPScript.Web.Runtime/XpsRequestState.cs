...
public sealed class XpsRequestState : IXpsRequestState
{
    ...
    public XpsRequestState(int maxEntries = 128, int maxValueBytes = 64 * 1024, long maxTotalBytes = 1024 * 1024)
    {
        ...
        if (maxTotalBytes < maxValueBytes || maxTotalBytes > 64L * 1024 * 1024) throw new ArgumentOutOfRangeException(nameof(maxTotalBytes));
    ...
    public void Set(string name, object? value)
    {
        ...
        lock (_gate)
        {
            ...
            if (previous is null && _values.Count >= _maxEntries)
                throw new InvalidOperationException("Request scope entry limit has been reached.");
            var nextTotal = checked(_totalBytes - (previous?.Bytes ?? 0) + stateValue.Bytes);
            if (nextTotal > _maxTotalBytes)
                throw new InvalidOperationException("Request scope memory limit has been reached.");
    ...
    private static void ValidateName(string name)
    {
        ...
        if (name.Length > 256) throw new ArgumentOutOfRangeException(nameof(name), "Request scope value name cannot exceed 256 characters.");
    }
}
