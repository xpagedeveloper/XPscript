...
{
    public const string BrowserWasmAssetRoute = "__browser_wasm_asset__";
    ...
    public XpsRouteResolution Resolve(string requestPath, Func<string, bool>? fileExists = null, Func<string, bool>? directoryExists = null)
    {
        ...
        var xpsSegment = segments.FindIndex(segment => segment.EndsWith(".xps", StringComparison.OrdinalIgnoreCase));
        if (xpsSegment >= 0 && xpsSegment < segments.Count - 1)
        {
            var scriptRelative = string.Join(Path.DirectorySeparatorChar, segments.Take(xpsSegment + 1));
            var script = ResolveCandidate(scriptRelative, BrowserWasmAssetRoute, fileExists);
        ...
        if (segments[^1].EndsWith(".xps", StringComparison.OrdinalIgnoreCase)) return ResolveCandidate(relative, null, fileExists);
    ...
    private XpsRouteResolution ResolveCandidate(string relativePath, string? routeFunction, Func<string, bool> fileExists)
    {
        ...
        return fileExists(fullPath) ? new XpsRouteResolution(fullPath, routeFunction, true) : XpsRouteResolution.NotFound;
    }
    ...
    private string MapInsideRoot(string relativePath)
    {
        ...
        if (!IsInsideRoot(full)) throw new XpsWebPathException("Resolved request path escapes the configured web root.");
    ...
    private static List<string> NormalizeUrlPath(string requestPath)
    {
        ...
        if (requestPath.IndexOf('\0') >= 0 || requestPath.Any(char.IsControl)) throw new XpsWebPathException("Request path contains a prohibited control character.");
        string decoded;
        try { decoded = Uri.UnescapeDataString(requestPath); }
        catch (UriFormatException ex) { throw new XpsWebPathException("Request path contains malformed percent encoding.", ex); }
        if (decoded.Contains('%'))
        {
    ...
    {
        if (string.IsNullOrWhiteSpace(value)) throw new ArgumentException("Default document name is required.", nameof(value));
        var normalized = value.Trim();
        if (!normalized.EndsWith(".xps", StringComparison.OrdinalIgnoreCase) || normalized.Length > 255 || normalized.IndexOfAny(['/', '\\', '\0', ':']) >= 0 || normalized.Any(char.IsControl) || normalized is "." or "..")
    ...
    private static void ValidateRouteFunction(string value)
    {
        ...
        if (value.Skip(1).Any(c => !(char.IsLetterOrDigit(c) || c == '_'))) throw new XpsWebPathException("Route function name is invalid.");
    }
