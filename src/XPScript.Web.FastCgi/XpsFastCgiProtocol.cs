...
internal static class XpsFastCgiProtocol
{
    ...
    internal static async ValueTask<Record?> ReadRecordAsync(Stream stream, CancellationToken cancellationToken)
    {
        ...
        if (headerRead != header.Length) throw new XpsFastCgiProtocolException("Truncated FastCGI record header.");
        if (header[0] != Version1) throw new XpsFastCgiProtocolException("Unsupported FastCGI version.");
        ...
        if (header[7] != 0) throw new XpsFastCgiProtocolException("FastCGI reserved header byte must be zero.");

        var content = new byte[contentLength];
        if (contentLength > 0) await ReadExactlyAsync(stream, content, cancellationToken).ConfigureAwait(false);
        if (paddingLength > 0)
        {
    ...
    internal static void ParseParams(ReadOnlySpan<byte> content, IDictionary<string, string> destination, XpsFastCgiOptions options, ref int totalParamsBytes, ref int totalParamCount)
    {
        totalParamsBytes = checked(totalParamsBytes + content.Length);
        if (totalParamsBytes > options.MaxParamsBytes) throw new XpsFastCgiProtocolException("FastCGI PARAMS exceed the configured size limit.");
        var offset = 0;
        while (offset < content.Length)
        {
            var nameLength = ReadLength(content, ref offset);
            var valueLength = ReadLength(content, ref offset);
            if (nameLength > options.MaxParamNameBytes) throw new XpsFastCgiProtocolException("FastCGI parameter name exceeds the configured limit.");
            if (valueLength > options.MaxParamValueBytes) throw new XpsFastCgiProtocolException("FastCGI parameter value exceeds the configured limit.");
            var required = checked(nameLength + valueLength);
            if (required > content.Length - offset) throw new XpsFastCgiProtocolException("Truncated FastCGI PARAMS name/value pair.");
            totalParamCount = checked(totalParamCount + 1);
            if (totalParamCount > options.MaxParamCount) throw new XpsFastCgiProtocolException("FastCGI parameter count exceeds the configured limit.");
            ...
            offset += valueLength;
            if (string.IsNullOrEmpty(name)) throw new XpsFastCgiProtocolException("FastCGI parameter name must not be empty.");
            if (destination.ContainsKey(name)) throw new XpsFastCgiProtocolException("Duplicate FastCGI parameter names are not accepted.");
            destination.Add(name, value);
        }
    }

    internal static async Task WriteRecordAsync(Stream stream, XpsFastCgiRecordType type, ushort requestId, ReadOnlyMemory<byte> content, CancellationToken cancellationToken)
    {
    ...
    internal static async Task WriteStreamAsync(Stream stream, XpsFastCgiRecordType type, ushort requestId, ReadOnlyMemory<byte> content, CancellationToken cancellationToken)
    {
        var offset = 0;
        while (offset < content.Length)
        {
    ...
    internal static async Task WriteEndRequestAsync(Stream stream, ushort requestId, uint appStatus, byte protocolStatus, CancellationToken cancellationToken)
    {
        var body = new byte[8];
    ...
    {
        if (offset >= content.Length) throw new XpsFastCgiProtocolException("Truncated FastCGI PARAMS length.");
        var first = content[offset++];
        if ((first & 0x80) == 0) return first;
        if (content.Length - offset < 3) throw new XpsFastCgiProtocolException("Truncated FastCGI four-byte PARAMS length.");
        var value = ((first & 0x7f) << 24) | (content[offset] << 16) | (content[offset + 1] << 8) | content[offset + 2];
        offset += 3;
        if (value < 0) throw new XpsFastCgiProtocolException("Invalid FastCGI PARAMS length.");
        return value;
    }
    ...
    private static string DecodeUtf8(ReadOnlySpan<byte> value, string field)
    {
        ...
        catch (DecoderFallbackException ex) { throw new XpsFastCgiProtocolException($"FastCGI {field} contains invalid UTF-8.", ex); }
    }
