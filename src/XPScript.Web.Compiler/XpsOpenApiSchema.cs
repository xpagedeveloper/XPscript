...
internal static class XpsOpenApiSchema
{
    ...
    {
        var previous = ReferenceTypeNames.Value;
        ReferenceTypeNames.Value = names;
        return new ReferenceTypeNameScope(previous);
    }

    private sealed class ReferenceTypeNameScope(IReadOnlyDictionary<string, string>? previous) : IDisposable
    {
        public void Dispose() => ReferenceTypeNames.Value = previous;
    }
    ...
    internal static string ReferenceTypeName(string reference, string context)
    {
        ...
        if (ReferenceTypeNames.Value is { } names && names.TryGetValue(raw, out var mapped)) return mapped;
