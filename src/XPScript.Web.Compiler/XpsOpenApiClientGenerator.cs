...
public sealed class XpsOpenApiClientGenerator
{
    ...
    private static string EmitSource(string version, string? sourceName, string apiName, string? baseUrl, JsonObject root, Dictionary<string, JsonObject> models, IReadOnlyList<ClientOperation> operations, Dictionary<string, ClientSecurityScheme> securitySchemes)
    {
        ...
        b.AppendLine($"Public Class {apiName}"); b.AppendLine("    Private BaseUrl_i As String"); b.AppendLine("    Private Http_i As XPHttpClient"); foreach (var scheme in securitySchemes.Values) { var n = scheme.GeneratedName; if (scheme.Kind == "basic") { b.AppendLine($"    Private Auth{n}_iAuthorization As String"); } else b.AppendLine($"    Private Auth{n}_i As String"); } b.AppendLine(); b.AppendLine("    Public Sub New(url As String)"); b.AppendLine("        BaseUrl_i = url"); b.AppendLine("        Set Http_i = New XPHttpClient"); b.AppendLine("    End Sub"); b.AppendLine();
    ...
    private static void EmitOperation(StringBuilder b, string apiName, ClientOperation op, JsonObject root, Dictionary<string, JsonObject> models, Dictionary<string, ClientSecurityScheme> securitySchemes, IReadOnlyDictionary<string, string> responseMembers)
    {
        ...
        var args = op.Parameters.Where(p => p.Required).Select(p => $"{p.GeneratedName} As {p.TypeName}").Concat(op.Parameters.Where(p => !p.Required).Select(p => $"Optional {p.GeneratedName} As Variant = Nothing")).ToList();
        var procedureNames = new HashSet<string>(op.Parameters.Select(p => p.GeneratedName), StringComparer.OrdinalIgnoreCase);
        var payloadName = op.Body is null ? "payload" : UniqueIdentifier("payload", procedureNames);
        if (op.Body is not null) args.Add(op.Body.Required ? $"{payloadName} As {op.Body.TypeName}" : $"Optional {payloadName} As Variant = Nothing");
        var urlName = UniqueIdentifier("url", procedureNames);
        var rawName = UniqueIdentifier("raw", procedureNames);
        var requestName = UniqueIdentifier("request", procedureNames);
        var resultName = UniqueIdentifier("result", procedureNames);
        b.AppendLine($"    Public Function {op.Name}({string.Join(", ", args)}) As {responseName}");
        b.AppendLine($"        Dim {urlName} As String");
        b.AppendLine($"        Dim {rawName} As XPHttpResponse");
        b.AppendLine($"        Dim {requestName} As New XPHttpRequest");
    ...
    private static void EmitResponseValidation(StringBuilder b, ClientOperation op, JsonObject root, string rawName, string resultName)
    {
        ...
        {
            b.AppendLine($"        {(first ? "If" : "ElseIf")} {rawName}.StatusCode = {response.Code} Then");
            b.AppendLine($"            If Not {resultName}.Json Is Nothing Then Set {resultName}.Validation = XPJsonSchema.Parse(\"{EscapeXps(ResponseSchemaText(root, response.Schema!))}\").Validate({resultName}.Json)");
            first = false;
        }
        ...
        if (fallback is not null)
        {
            ...
            b.AppendLine($"            If Not {resultName}.Json Is Nothing Then Set {resultName}.Validation = XPJsonSchema.Parse(\"{EscapeXps(ResponseSchemaText(root, fallback.Schema!))}\").Validate({resultName}.Json)");
            first = false;
        }
