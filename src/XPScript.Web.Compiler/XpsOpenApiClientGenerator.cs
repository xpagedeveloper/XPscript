...
public sealed class XpsOpenApiClientGenerator
{
    ...
    private static Dictionary<string, JsonObject> CollectModels(JsonObject root) { var result = new Dictionary<string, JsonObject>(StringComparer.OrdinalIgnoreCase); if (root["components"] is not JsonObject components || components["schemas"] is not JsonObject schemas) return result; foreach (var pair in schemas) if (pair.Value is JsonObject schema) result.Add(ToIdentifier(pair.Key), schema); return result; }
    private static Dictionary<string, ClientSecurityScheme> CollectSecuritySchemes(JsonObject root)
    {
        ...
        {
            var scheme = Resolve(root, pair.Value); var type = ReadString(scheme, "type")?.ToLowerInvariant();
    ...
    private static string EmitSource(string version, string? sourceName, string apiName, string? baseUrl, JsonObject root, Dictionary<string, JsonObject> models, IReadOnlyList<ClientOperation> operations, Dictionary<string, ClientSecurityScheme> securitySchemes)
    {
        ...
        var responseName = apiName + "Response"; b.AppendLine($"Public Class {responseName}"); b.AppendLine("    Public StatusCode As Integer"); b.AppendLine("    Public IsSuccess As Boolean"); b.AppendLine("    Public ResponseType As String"); b.AppendLine("    Public Raw As XPHttpResponse"); b.AppendLine("    Public Json As XPJsonDocument");
        ...
        b.AppendLine($"Public Class {apiName}"); b.AppendLine("    Private BaseUrl As String"); b.AppendLine("    Private Http As XPHttpClient"); foreach (var scheme in securitySchemes.Values) { var n = ToIdentifier(scheme.Name); if (scheme.Kind == "basic") { b.AppendLine($"    Private Auth{n}Username As String"); b.AppendLine($"    Private Auth{n}Password As String"); } else b.AppendLine($"    Private Auth{n} As String"); } b.AppendLine(); b.AppendLine("    Public Sub New(url As String)"); b.AppendLine("        BaseUrl = url"); b.AppendLine("        Set Http = New XPHttpClient"); b.AppendLine("    End Sub"); b.AppendLine();
        ...
        foreach (var scheme in securitySchemes.Values) { var n = ToIdentifier(scheme.Name); if (scheme.Kind == "basic") { b.AppendLine($"    Public Sub Set{n}(username As String, password As String)"); b.AppendLine($"        Auth{n}Username = username"); b.AppendLine($"        Auth{n}Password = password"); } else { b.AppendLine($"    Public Sub Set{n}(value As String)"); b.AppendLine($"        Auth{n} = value"); } b.AppendLine("    End Sub"); b.AppendLine(); } b.AppendLine("    Public Sub SetBearerToken(token As String)"); b.AppendLine("        Call Http.SetBearerToken(token)"); b.AppendLine("    End Sub"); b.AppendLine(); b.AppendLine("    Public Sub SetBasicAuth(username As String, password As String)"); b.AppendLine("        Call Http.SetBasicAuth(username, password)"); b.AppendLine("    End Sub"); b.AppendLine(); b.AppendLine("    Public Sub SetHeader(name As String, value As String)"); b.AppendLine("        Call Http.SetHeader(name, value)"); b.AppendLine("    End Sub"); b.AppendLine(); foreach (var op in operations) { EmitOperation(b, apiName, op, models, securitySchemes); b.AppendLine(); } b.AppendLine("End Class"); b.AppendLine("' </xpscript-openapi-client>"); return b.ToString();
    }
