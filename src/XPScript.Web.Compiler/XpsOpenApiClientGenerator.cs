...
public sealed class XpsOpenApiClientGenerator
{
    ...
    {
        var responseName = apiName + "Response"; var args = op.Parameters.Where(p => p.Required).Select(p => $"{p.GeneratedName} As {p.TypeName}").Concat(op.Parameters.Where(p => !p.Required).Select(p => $"Optional {p.GeneratedName} As Variant = Nothing")).ToList(); var procedureNames = new HashSet<string>(op.Parameters.Select(p => p.GeneratedName), StringComparer.OrdinalIgnoreCase); var payloadName = op.Body is null ? "payload" : UniqueIdentifier("payload", procedureNames); if (op.Body is not null) args.Add(op.Body.Required ? $"{payloadName} As {op.Body.TypeName}" : $"Optional {payloadName} As Variant = Nothing"); var urlName = UniqueIdentifier("url", procedureNames); var rawName = UniqueIdentifier("raw", procedureNames); var requestName = UniqueIdentifier("request", procedureNames); var resultName = UniqueIdentifier("result", procedureNames); b.AppendLine($"    Public Function {op.Name}({string.Join(", ", args)}) As {responseName}"); b.AppendLine($"        Dim {urlName} As String"); b.AppendLine($"        Dim {rawName} As XPHttpResponse"); b.AppendLine($"        Dim {requestName} As New XPHttpRequest"); b.AppendLine($"        Dim {resultName} As {responseName}"); b.AppendLine($"        Set {resultName} = New {responseName}"); b.AppendLine($"        {urlName} = BaseUrl & \"{EscapeXps(op.Path)}\"");
