...
public sealed class XpsOpenApiClientGenerator
{
    ...
    private static string EmitSource(string version, string? sourceName, string apiName, string? baseUrl, JsonObject root, Dictionary<string, JsonObject> models, IReadOnlyList<ClientOperation> operations, Dictionary<string, ClientSecurityScheme> securitySchemes)
    {
        ...
        foreach (var scheme in securitySchemes.Values) { var n = ToIdentifier(scheme.Name); if (scheme.Kind == "basic") { b.AppendLine($"    Public Sub Set{n}(username As String, password As String)"); b.AppendLine($"        Auth{n}Username = username"); b.AppendLine($"        Auth{n}Password = password"); } else { b.AppendLine($"    Public Sub Set{n}(value As String)"); b.AppendLine($"        Auth{n} = value"); } b.AppendLine("    End Sub"); b.AppendLine(); } b.AppendLine("    Public Sub SetHeader(name As String, value As String)"); b.AppendLine("        Call Http.SetHeader(name, value)"); b.AppendLine("    End Sub"); b.AppendLine(); foreach (var op in operations) { EmitOperation(b, apiName, op, models, securitySchemes); b.AppendLine(); } b.AppendLine("End Class"); b.AppendLine("' </xpscript-openapi-client>"); return b.ToString();
    }
