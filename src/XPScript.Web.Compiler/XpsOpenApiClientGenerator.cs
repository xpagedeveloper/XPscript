...
public sealed class XpsOpenApiClientGenerator
{
    ...
    private static string EmitSource(string version, string? sourceName, string apiName, string? baseUrl, JsonObject root, Dictionary<string, JsonObject> models, IReadOnlyList<ClientOperation> operations, Dictionary<string, ClientSecurityScheme> securitySchemes)
    {
        ...
        foreach (var scheme in securitySchemes.Values) { var n = ToIdentifier(scheme.Name); if (scheme.Kind == "basic") { b.AppendLine($"    Public Sub Set{n}(username As String, password As String)"); b.AppendLine($"        Auth{n}Authorization = Http.BasicAuthorization(username, password)"); } else { b.AppendLine($"    Public Sub Set{n}(value As String)"); b.AppendLine($"        Auth{n} = value"); } b.AppendLine("    End Sub"); b.AppendLine(); } b.AppendLine("    Public Sub SetHeader(name As String, value As String)"); b.AppendLine("        Call Http.SetHeader(name, value)"); b.AppendLine("    End Sub"); b.AppendLine(); foreach (var op in operations) { EmitOperation(b, apiName, op, models, securitySchemes); b.AppendLine(); } b.AppendLine("End Class"); b.AppendLine("' </xpscript-openapi-client>"); return b.ToString();
    }
    ...
    private static string XpsType(JsonObject root, JsonObject schema) { if (schema["type"] is JsonArray types) { var nonNull = types.Select(x => x?.GetValue<string>()).Where(x => !string.Equals(x, "null", StringComparison.OrdinalIgnoreCase)).Distinct(StringComparer.OrdinalIgnoreCase).ToArray(); if (nonNull.Length == 1) { var copy = (JsonObject)schema.DeepClone(); copy["type"] = nonNull[0]; return XpsType(root, copy); } return "Variant"; } if (ReadString(schema, "$ref") is { } reference) { _ = Resolve(root, schema); return ToIdentifier(reference[(reference.LastIndexOf('/') + 1)..]); } var type = ReadString(schema, "type")?.ToLowerInvariant(); var format = ReadString(schema, "format")?.ToLowerInvariant(); return type switch { "integer" => format == "int32" ? "Integer" : "Long", "number" => format == "float" ? "Single" : "Double", "boolean" => "Boolean", "string" => format is "date" or "date-time" ? "Date" : "String", "array" => "XPJsonArray", "object" => "XPJsonObject", _ => "Variant" }; }
