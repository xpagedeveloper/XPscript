...
public sealed class XpsOpenApiImporter
{
    ...
    public XpsOpenApiImportResult Import(string specification, string existingSource, string? sourceName = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(existingSource) && !ContainsGeneratedSource(existingSource, sourceName))
    ...
    private static bool ContainsGeneratedSource(string source, string? sourceName)
    {
        if (string.IsNullOrWhiteSpace(sourceName)) return false;
        var marker = " - " + sourceName;
        return source.Split(new[] { "\r\n", "\n" }, StringSplitOptions.None)
            .Any(line => line.StartsWith("' Generated by XPScript from OpenAPI ", StringComparison.OrdinalIgnoreCase)
                      && line.EndsWith(marker, StringComparison.OrdinalIgnoreCase));
    }
