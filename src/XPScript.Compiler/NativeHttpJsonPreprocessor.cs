...
internal sealed class NativeHttpJsonPreprocessor
{
    ...
    public string Transform(string source)
    {
        ...
        foreach (var raw in lines)
        {
            ...
            if (dimNew.Success)
            {
                var name = dimNew.Groups[1].Value; var type = dimNew.Groups[2].Value;
                nativeVariables.Add(name); nativeTypes[name] = type;
            ...
            if (dim.Success)
            {
                var name = dim.Groups[1].Value; var type = dim.Groups[2].Value;
                nativeVariables.Add(name); nativeTypes[name] = type;
                output.Add(indent + $"Dim {name} As Variant");
                continue;
            }
            ...
            foreach (var pair in nativeTypes)
            {
                ...
                if (pair.Value.Equals("XPHttpClient", StringComparison.OrdinalIgnoreCase))
                {
                    foreach (var method in new[] { "GetJson", "PostJson", "PutJson", "PatchJson", "PostForm", "AddQuery", "LoadForm", "SaveForm", "PutForm" })
                        rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.{method}\s*\(", $"XPScriptHttpUiFormHelpers.{method}({pair.Key}, ", RegexOptions.IgnoreCase);
                    foreach (var method in new[] { "GetAsync", "DeleteAsync", "PostAsync", "PutAsync", "PatchAsync" })
                        rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.{method}\s*\(", $"XPScriptAsyncHttp.{method}({pair.Key}, ", RegexOptions.IgnoreCase);
                }
                else if (pair.Value.Equals("XPHttpResponse", StringComparison.OrdinalIgnoreCase))
                    rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.Json\s*\(\s*\)", $"XPScriptHttpUiFormHelpers.ResponseJson({pair.Key})", RegexOptions.IgnoreCase);
                else if (pair.Value.Equals("XPDBSQLite", StringComparison.OrdinalIgnoreCase))
                {
                    foreach (var method in new[] { "QueryArray", "GetRow", "SaveRow" }) rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.{method}\s*\(", $"XPScriptSqliteDataSourceExtensions.{method}({pair.Key}, ", RegexOptions.IgnoreCase);
                    rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.Attachments\s*\(", $"XPScriptDatabaseAttachmentRuntime.ForSqlite({pair.Key}, ", RegexOptions.IgnoreCase);
                }
                else if (pair.Value.Equals("XPDbMsSql", StringComparison.OrdinalIgnoreCase))
                {
                    foreach (var method in new[] { "QueryArray", "GetRow", "SaveRow" }) rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.{method}\s*\(", $"XPScriptMsSqlDataSourceExtensions.{method}({pair.Key}, ", RegexOptions.IgnoreCase);
                    rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.Attachments\s*\(", $"XPScriptDatabaseAttachmentRuntime.ForMsSql({pair.Key}, ", RegexOptions.IgnoreCase);
                }
                else if (pair.Value.Equals("XPHttpDbSupabase", StringComparison.OrdinalIgnoreCase))
                {
                    foreach (var method in new[] { "QueryArray", "GetRow", "SaveRow" }) rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.{method}\s*\(", $"XPScriptHttpDatabaseDataSourceExtensions.{method}({pair.Key}, ", RegexOptions.IgnoreCase);
                    rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.Attachments\s*\(", $"XPScriptDatabaseAttachmentRuntime.ForSupabase({pair.Key}, ", RegexOptions.IgnoreCase);
                    rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.SetAttachmentBucket\s*\(", $"XPScriptDatabaseAttachmentRuntime.SetSupabaseBucket({pair.Key}, ", RegexOptions.IgnoreCase);
                }
                else if (pair.Value.Equals("XPHttpDbDominoRest", StringComparison.OrdinalIgnoreCase))
                {
                    foreach (var method in new[] { "GetViewArray", "QueryArray", "GetRow", "SaveRow" }) rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.{method}\s*\(", $"XPScriptHttpDatabaseDataSourceExtensions.{method}({pair.Key}, ", RegexOptions.IgnoreCase);
                    rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.Attachments\s*\(", $"XPScriptDatabaseAttachmentRuntime.ForDomino({pair.Key}, ", RegexOptions.IgnoreCase);
                }
                else if (pair.Value.Equals("XPAi", StringComparison.OrdinalIgnoreCase))
                {
                    foreach (var method in new[] { "AddTool", "RemoveTool", "HasTool", "GetTool", "ClearTools", "GetToolNames", "ToolCount" })
                        rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.{method}\s*\(", $"XPScriptAiToolRegistry.{method}({pair.Key}, ", RegexOptions.IgnoreCase);
                    rewritten = rewritten.Replace($"XPScriptAiToolRegistry.ClearTools({pair.Key}, )", $"XPScriptAiToolRegistry.ClearTools({pair.Key})", StringComparison.OrdinalIgnoreCase)
