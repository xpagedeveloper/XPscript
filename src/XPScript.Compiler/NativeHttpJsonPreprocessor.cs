    ...
    public string Transform(string source)
    {
        ...
        foreach (var raw in lines)
        {
            ...
            foreach (var pair in nativeTypes)
            {
                ...
                {
                    foreach (var method in new[] { "GetJson", "PostJson", "PutJson", "PatchJson", "PostForm", "AddQuery", "LoadForm", "SaveForm", "PutForm", "SetBearerToken", "SetBasicAuth" }) rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.{method}\s*\(", $"XPScriptHttpUiFormHelpers.{method}({pair.Key}, ", RegexOptions.IgnoreCase);
                ...
                else if (pair.Value.Equals("XPHttpResponse", StringComparison.OrdinalIgnoreCase)) rewritten = Regex.Replace(rewritten, $@"\b{escapedName}\.Json\s*\(\s*\)", $"XPScriptHttpUiFormHelpers.ResponseJson({pair.Key})", RegexOptions.IgnoreCase);
