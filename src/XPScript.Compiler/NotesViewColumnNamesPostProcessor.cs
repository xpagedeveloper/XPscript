...
{
    public static string Apply(string source) => ApplyCore(source, "nint");
    public static string ApplyBuiltSurface(string source) => ApplyCore(source, "uint");

    private static string ApplyCore(string source, string handleType)
    {
        ArgumentNullException.ThrowIfNull(source);

        source = ReplaceRequired(
            source,
            $"    internal {handleType} NativeHandle {{ get {{ EnsureAlive(); return _handle; }} }}\n    public string Name {{ get; }}",
