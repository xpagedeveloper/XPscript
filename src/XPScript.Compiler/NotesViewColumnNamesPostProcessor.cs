...
internal static class NotesViewColumnNamesPostProcessor
{
    ...
    private static string ApplyCore(string source, string viewHandleType, string databaseHandleType)
    {
        ...
        source = ReplaceRequired(
            ...
            $"    internal {viewHandleType} NativeHandle {{ get {{ EnsureAlive(); return _handle; }} }}\n    public string Name {{ get; }}\n    public LSArray ColumnNames\n    {{\n        get\n        {{\n            EnsureAlive();\n            var names = Session.Api.GetViewColumnNames(Database.Handle, Name);\n            if (names.Length == 0) return new LSArray(\"String\", true);\n            var result = new LSArray(\"String\", true, [0], [names.Length - 1]);\n            for (var i = 0; i < names.Length; i++) result.Set(names[i], i);\n            return result;\n        }}\n    }}",
        ...
        if (!source.Contains("public LSArray ColumnNames", StringComparison.Ordinal))
            throw new CompilerException("NotesView.ColumnNames must expose an XPScript LSArray.");
