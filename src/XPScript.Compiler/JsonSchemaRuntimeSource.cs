...
internal sealed class XPScriptJsonSchema : IXPScriptJsonNodeConvertible
{
    ...
    private void SetKeyword(string name, object? value) { if (value is null || XPScriptNullRuntime.IsNull(value)) { _schema.Remove(name); return; } var node = value is XPScriptJsonSchema schema ? schema.ToJsonSchemaObject() : XPScriptNativeJson.ToNode(value); if (IsDirectSchemaKeyword(name) && node is not null && IsSchemaNode(node)) node = NormalizeSchemaNode(node); _schema[name] = node; XPScriptNativeJson.ValidateBudget(_schema); }
...
internal static class XPScriptJsonSchemaValidator
{
    ...
    private static void ValidateNode(System.Text.Json.Nodes.JsonObject schema, System.Text.Json.Nodes.JsonNode? node, string path, System.Text.Json.Nodes.JsonArray errors, int depth)
    {
        ...
        if (!MatchesTypeKeyword(node, schema["type"], out var expectedType)) { Add(errors, path, "type", "Value does not match the required JSON type.", expectedType, ActualType(node)); return; }

        if (schema["allOf"] is System.Text.Json.Nodes.JsonArray allOf)
        {
            for (var i = 0; i < allOf.Count; i++) if (allOf[i] is System.Text.Json.Nodes.JsonObject childSchema && !IsValidAgainst(childSchema, node, path, depth + 1)) Add(errors, path, "allOf", "Value does not satisfy every allOf schema.", "all schemas", "schema " + i + " failed");
        }
