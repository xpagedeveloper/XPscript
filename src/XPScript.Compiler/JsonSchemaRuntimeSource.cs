...
internal sealed class XPScriptJsonSchema
{
    ...
    public static XPScriptJsonSchema FromJson(object? value) { var node = XPScriptNativeJson.ToNode(value); if (node is not System.Text.Json.Nodes.JsonObject obj) throw new XPScriptRuntimeException(13, "XPJsonSchema requires a JsonObject or JsonDocument with an object root."); return new XPScriptJsonSchema(obj); }
    public static XPScriptJsonSchema Infer(object? value) => Infer(value, true);
    public static XPScriptJsonSchema Infer(object? value, object? requiredValue) { var node = XPScriptNativeJson.ToNode(value); if (node is null) throw new XPScriptRuntimeException(13, "XPJsonSchema.Infer requires an XPJson value."); XPScriptNativeJson.ValidateBudget(node); return new XPScriptJsonSchema(InferNode(node, requiredValue is null || XPScriptNullRuntime.IsNull(requiredValue) || XPScriptRuntime.CBool(requiredValue), 0)); }
