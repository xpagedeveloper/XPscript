...
internal static class NotesRuntimeSourceBuilder
{
    public static string Build()
    {
        ...
        source = ReplaceRequired(source,
            ...
            "document-items-property");

        source = ReplaceRequired(source,
            "    internal XPScriptNotesTimeDate GetItemModifiedTime(nint note, string name)",
            "    internal string[] GetItemNames(nint note)\n    {\n        EnsureInitialized();\n        var names = new List<string>();\n        var status = Resolve<NSFItemInfoDelegate>(\"NSFItemInfo\")(\n            note, 0, 0, out var itemBlock, out _, out _, out _);\n        if ((status & ErrMask) == ErrItemNotFound) return [];\n        Check(status, \"NSFItemInfo(first item)\");\n\n        const int nameBufferSize = 65536;\n        var nameBuffer = System.Runtime.InteropServices.Marshal.AllocHGlobal(nameBufferSize);\n        try\n        {\n            while (true)\n            {\n                Zero(nameBuffer, nameBufferSize);\n                Resolve<NSFItemQueryDelegate>(\"NSFItemQuery\")(\n                    note, itemBlock, nameBuffer, ushort.MaxValue,\n                    out var nameLength, out _, out _, out _, out _);\n                names.Add(nameLength == 0 ? \"\" : FromLmbcs(nameBuffer, nameLength));\n\n                status = Resolve<NSFItemInfoNextDelegate>(\"NSFItemInfoNext\")(\n                    note, itemBlock, 0, 0, out var nextItemBlock, out _, out _, out _);\n                if ((status & ErrMask) == ErrItemNotFound) break;\n                Check(status, \"NSFItemInfoNext\");\n                itemBlock = nextItemBlock;\n            }\n        }\n        finally\n        {\n            System.Runtime.InteropServices.Marshal.FreeHGlobal(nameBuffer);\n        }\n        return names.ToArray();\n    }\n\n    internal XPScriptNotesTimeDate GetItemModifiedTime(nint note, string name)",
            "document-item-name-enumeration");
