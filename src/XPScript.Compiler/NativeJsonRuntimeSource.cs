...
internal static class XPScriptJsonSchemaTypeGenerator
{
    ...
    private static System.Text.Json.Nodes.JsonObject Build(Type type, HashSet<Type> visiting, int depth)
    {
        ...
        if (type == typeof(System.Text.Json.Nodes.JsonArray)) return new System.Text.Json.Nodes.JsonObject { ["type"] = "array", ["items"] = new System.Text.Json.Nodes.JsonObject() };
        ...
        try
        {
            ...
            foreach (var field in type.GetFields(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance).OrderBy(field => field.MetadataToken)) { if (field.IsStatic) continue; var name = JsonName(field); if (!names.Add(name)) continue; properties[name] = Build(field.FieldType, visiting, depth + 1); required.Add(name); }
            foreach (var property in type.GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance).OrderBy(property => property.MetadataToken)) { if (!property.CanRead || property.GetMethod is null || !property.GetMethod.IsPublic || property.GetIndexParameters().Length != 0) continue; var name = JsonName(property); if (!names.Add(name)) continue; properties[name] = Build(property.PropertyType, visiting, depth + 1); required.Add(name); }
    ...
    private static string JsonName(System.Reflection.MemberInfo member)
    {
        var alias = member.GetCustomAttributes(typeof(System.Text.Json.Serialization.JsonPropertyNameAttribute), inherit: true).OfType<System.Text.Json.Serialization.JsonPropertyNameAttribute>().FirstOrDefault()?.Name;
        if (!string.IsNullOrEmpty(alias)) return alias;
        var name = member.Name;
        return name.Length == 0 || char.IsLower(name[0]) ? name : name.Length == 1 ? name.ToLowerInvariant() : char.ToLowerInvariant(name[0]) + name[1..];
    }
}
...
internal static class XPScriptNativeJson
{
    ...
    private static readonly System.Text.Json.JsonSerializerOptions SerializerOptions = new(System.Text.Json.JsonSerializerDefaults.Web) { MaxDepth = MaxDepth, IncludeFields = true, PropertyNameCaseInsensitive = true, Converters = { new System.Text.Json.Serialization.JsonStringEnumConverter(), new LSRefJsonConverterFactory(), new LSObjectJsonConverterFactory() } };
    ...
    internal static System.Text.Json.Nodes.JsonNode? ToNode(object? value)
    {
        ...
        try
        {
            node = value switch
            {
                null => null, _ when XPScriptNullRuntime.IsNull(value) => null, ILSObjectReference reference when reference.IsNothing => null,
                ILSObjectReference reference => ToNode(reference.ObjectValue), LSArray array => ArrayToNode(array), XPScriptJsonDocument document => document.Node?.DeepClone(),
                XPScriptJsonObject obj => obj.Node.DeepClone(), XPScriptJsonArray array => array.Node.DeepClone(), XPScriptJsonElement element => element.Node?.DeepClone(),
                IXPScriptJsonNodeConvertible convertible => convertible.ToJsonNode()?.DeepClone(), System.Text.Json.Nodes.JsonNode jsonNode => jsonNode.DeepClone(),
                string s => System.Text.Json.Nodes.JsonValue.Create(s), bool b => System.Text.Json.Nodes.JsonValue.Create(b), byte n => System.Text.Json.Nodes.JsonValue.Create(n), short n => System.Text.Json.Nodes.JsonValue.Create(n), int n => System.Text.Json.Nodes.JsonValue.Create(n), long n => System.Text.Json.Nodes.JsonValue.Create(n),
                float n => CreateFiniteNumber(n), double n => CreateFiniteNumber(n), decimal n => System.Text.Json.Nodes.JsonValue.Create(n), DateTime dt => System.Text.Json.Nodes.JsonValue.Create(dt),
                _ => System.Text.Json.JsonSerializer.SerializeToNode(value, value.GetType(), SerializerOptions)
            };
        }
        catch (System.Text.Json.JsonException) { throw new XPScriptRuntimeException(5, "Value cannot be converted to JSON within the supported depth limit."); }
...
{
    internal XPScriptJsonDocument(System.Text.Json.Nodes.JsonNode? node) { XPScriptNativeJson.ValidateBudget(node); Node = node; }
...
{
    internal XPScriptJsonObject(System.Text.Json.Nodes.JsonObject node) { XPScriptNativeJson.ValidateBudget(node); Node = node; }
    ...
    public void Set(object? name, object? value) { var key = XPScriptRuntime.CStr(name); var hadPrevious = Node.TryGetPropertyValue(key, out var previous); var previousCopy = previous?.DeepClone(); Node[key] = XPScriptNativeJson.ToNode(value); try { XPScriptNativeJson.ValidateBudget(Node); } catch { if (hadPrevious) Node[key] = previousCopy; else Node.Remove(key); throw; } }
...
{
    internal XPScriptJsonArray(System.Text.Json.Nodes.JsonArray node) { XPScriptNativeJson.ValidateBudget(node); Node = node; }
    internal System.Text.Json.Nodes.JsonArray Node { get; }
    public int Count => Node.Count;
    public void Add(object? value) { Node.Add(XPScriptNativeJson.ToNode(value)); try { XPScriptNativeJson.ValidateBudget(Node); } catch { Node.RemoveAt(Node.Count - 1); throw; } }
    public object? Get(object? indexValue) { var index = XPScriptRuntime.CInt(indexValue); if (index < 0 || index >= Node.Count) throw new XPScriptRuntimeException(9, "JSON array index out of range."); return XPScriptNativeJson.FromNode(Node[index]); }
...
{
    internal XPScriptJsonElement(System.Text.Json.Nodes.JsonNode? node) { XPScriptNativeJson.ValidateBudget(node); Node = node; }
    internal System.Text.Json.Nodes.JsonNode? Node { get; }
    public string Type => Node switch { null => "Null", System.Text.Json.Nodes.JsonObject => "Object", System.Text.Json.Nodes.JsonArray => "Array", System.Text.Json.Nodes.JsonValue value when value.TryGetValue<bool>(out _) => "Boolean", System.Text.Json.Nodes.JsonValue value when value.TryGetValue<string>(out _) => "String", System.Text.Json.Nodes.JsonValue => "Number", _ => "Null" };
