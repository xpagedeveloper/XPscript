...
internal sealed class ClassOverloadValidator
{
    ...
    {
        ["Byte"] = 0, ["Integer"] = 1, ["Long"] = 2, ["Single"] = 3, ["Double"] = 4, ["Currency"] = 5
    };
    ...
    private static List<Method> CollectMethods(string[] lines)
    {
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            var methodMatch = Regex.Match(line, @"^(?:(?:Public|Private|Static)\s+)?(Sub|Function)\s+([A-Za-z_]\w*)\s*\((.*)\)", RegexOptions.IgnoreCase);
            ...
            result.Add(new Method(currentClass, name, methodMatch.Groups[1].Value.Equals("Function", StringComparison.OrdinalIgnoreCase), ParseParameters(methodMatch.Groups[3].Value), i + 1));
        }
        return result;
    }
    ...
    private static IReadOnlyList<Parameter> ParseParameters(string text)
    {
        ...
        foreach (var raw in SplitArguments(text))
        {
            ...
            var match = Regex.Match(raw.Trim(), @"^(?<mods>(?:(?:Optional|ByVal|ByRef)\s+)*)?(?<name>[A-Za-z_]\w*)\s*(?<array>\(\))?\s*(?:As\s+(?<type>[A-Za-z_]\w*))?(?:\s*=\s*.+)?$", RegexOptions.IgnoreCase);
            if (!match.Success) continue;
            var modifiers = match.Groups["mods"].Value;
            result.Add(new Parameter(match.Groups["name"].Value, NormalizeType(match.Groups["type"].Success ? match.Groups["type"].Value : "Variant"), match.Groups["array"].Success, Regex.IsMatch(modifiers, @"\bOptional\b", RegexOptions.IgnoreCase), Regex.IsMatch(modifiers, @"\bByRef\b", RegexOptions.IgnoreCase)));
        }
        return result;
    }
    ...
    private static void ValidateCalls(string[] lines, IReadOnlyList<Method> methods, string sourceName)
    {
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            if (classMatch.Success) { currentClass = classMatch.Groups[1].Value; continue; }
            if (Regex.IsMatch(line, @"^End\s+Class$", RegexOptions.IgnoreCase)) { currentClass = null; continue; }
            var procMatch = Regex.Match(line, @"^(?:(?:Public|Private|Static)\s+)?(?:Sub|Function)\s+[A-Za-z_]\w*\s*\((.*)\)", RegexOptions.IgnoreCase);
            if (procMatch.Success)
            {
                inProcedure = true; variables.Clear();
                foreach (var p in ParseParameters(procMatch.Groups[1].Value)) variables[p.Name] = (p.Type, p.IsArray);
                continue;
            }
            if (Regex.IsMatch(line, @"^End\s+(?:Sub|Function)$", RegexOptions.IgnoreCase)) { inProcedure = false; variables.Clear(); continue; }
            if (!inProcedure) continue;
            var dim = Regex.Match(line, @"^Dim\s+([A-Za-z_]\w*)\s*(\(.*\))?\s+As\s+(?:New\s+)?([A-Za-z_]\w*)", RegexOptions.IgnoreCase);
            if (dim.Success) variables[dim.Groups[1].Value] = (NormalizeType(dim.Groups[3].Value), dim.Groups[2].Success);
            foreach (var call in FindCalls(line, currentClass, variables))
            {
                var candidates = methods.Where(m => m.ClassName.Equals(call.ClassName, StringComparison.OrdinalIgnoreCase) && m.Name.Equals(call.MethodName, StringComparison.OrdinalIgnoreCase)).ToArray();
    ...
    private static IEnumerable<CallSite> FindCalls(string line, string? currentClass, IReadOnlyDictionary<string, (string Type, bool IsArray)> variables)
    {
        foreach (Match match in Regex.Matches(line, @"\b(?<target>[A-Za-z_]\w*)\.(?<name>[A-Za-z_]\w*)\s*\((?<args>[^()]*)\)", RegexOptions.IgnoreCase))
        {
            ...
            else if (variables.TryGetValue(target, out var variable) && !variable.IsArray)
                yield return new CallSite(variable.Type, name, target + "." + name, SplitArguments(match.Groups["args"].Value));
        }
    }

    private static void ResolveCall(IReadOnlyList<Method> candidates, IReadOnlyList<string> arguments, Dictionary<string, (string Type, bool IsArray)> variables, string sourceName, int lineNumber, string original, string displayName)
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            for (var index = 0; index < arguments.Count; index++)
            {
                ...
                if (parameter.IsByRef && !IsSimpleVariable(argument)) { valid = false; break; }
                var score = MatchScore(parameter, InferType(argument, variables));
                if (score < 0) { valid = false; break; }
                total += score;
            }
    ...
    private static int MatchScore(Parameter parameter, (string Type, bool IsArray)? actual)
    {
        ...
        var expected = NormalizeType(parameter.Type); var actualType = NormalizeType(actual.Value.Type);
        ...
        if (NumericRank.TryGetValue(expected, out var er) && NumericRank.TryGetValue(actualType, out var ar)) return er >= ar ? 1 + er - ar : 8 + ar - er;
        return -1;
    }
    ...
    private static bool IsScalarBuiltIn(string type) => type.Equals("String", StringComparison.OrdinalIgnoreCase) || type.Equals("Date", StringComparison.OrdinalIgnoreCase) || type.Equals("Boolean", StringComparison.OrdinalIgnoreCase) || NumericRank.ContainsKey(type);
    private static string EffectiveClrType(string type) => type.Equals("Variant", StringComparison.OrdinalIgnoreCase) || type.Equals("Object", StringComparison.OrdinalIgnoreCase) ? "Object" : NormalizeType(type);
    private static string NormalizeType(string type) => type.Trim() switch { var x when x.Equals("Int", StringComparison.OrdinalIgnoreCase) => "Integer", var x when x.Equals("Bool", StringComparison.OrdinalIgnoreCase) => "Boolean", _ => type.Trim() };

    private static List<string> SplitArguments(string value)
    {
        var result = new List<string>(); if (string.IsNullOrWhiteSpace(value)) return result;
        var start = 0; var inString = false; var depth = 0;
        for (var i = 0; i < value.Length; i++)
        {
            if (value[i] == '"') { if (inString && i + 1 < value.Length && value[i + 1] == '"') { i++; continue; } inString = !inString; continue; }
            if (inString) continue;
            if (value[i] == '(') depth++; else if (value[i] == ')') depth--; else if (value[i] == ',' && depth == 0) { result.Add(value[start..i].Trim()); start = i + 1; }
        }
        result.Add(value[start..].Trim()); return result;
    }
