...
""",
            """
            sb.Append("const applyState=s=>{if(!s)return;const cols=Array.isArray(s.columns)?s.columns:[];const rows=Array.isArray(s.rows)?s.rows:[];const hr=root.querySelector('thead tr');if(hr){hr.innerHTML='';cols.forEach((c,i)=>{const th=document.createElement('th');if(Number(c.width)>0)th.style.width=Number(c.width)+'px';if(s.sortable){const b=document.createElement('button');b.type='button';b.className='xps-list-sort';b.dataset.column=String(i);b.textContent=c.label??c.name??'';th.appendChild(b);}else th.textContent=c.label??c.name??'';hr.appendChild(th);});}body.innerHTML='';rows.forEach(r=>{const tr=document.createElement('tr');tr.dataset.rowIndex=String(r.index);if(r.href){tr.dataset.href=r.href;tr.tabIndex=0;tr.setAttribute('role','link');}(Array.isArray(r.values)?r.values:[]).forEach(v=>{const td=document.createElement('td');td.textContent=v??'';tr.appendChild(td);});body.appendChild(tr);});bindSort();};const postEvent=async(k,r)=>{const p=new URLSearchParams();p.set('__xps_list_event',k);p.set('__xps_list_index',r?.dataset.rowIndex||'');const x=await fetch(location.href,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},credentials:'same-origin',body:p.toString()});if(!x.ok)throw new Error('UIListView event failed');const text=await x.text();if(text){const state=JSON.parse(text);applyState(state);}return true;};");
""");
...
""");
    ...
    private static string ReplaceRequired(string source, string oldValue, string newValue)
    {
        if (!source.Contains(oldValue, StringComparison.Ordinal))
            throw new CompilerException("Unable to install UIListView live update runtime.");
