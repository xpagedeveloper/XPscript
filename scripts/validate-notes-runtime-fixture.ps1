...
if ($dxl.DocumentElement.LocalName -ne 'database') {
    throw "DXL root must be <database>, got <$($dxl.DocumentElement.LocalName)>"
}
if ($dxl.DocumentElement.NamespaceURI -ne $expectedNamespace) {
    throw "Unexpected DXL namespace: $($dxl.DocumentElement.NamespaceURI)"
}
...
function Require-SingleNode {
    ...
    if ($nodes.Count -ne 1) {
        throw "$Label must occur exactly once in the DXL fixture, found $($nodes.Count)."
    }
    return $nodes[0]
}
...
if ($defaultFormula.InnerText.Trim() -ne '"DEFAULT-VALUE"') {
    throw "CWFDefault formula must return DEFAULT-VALUE, got: $($defaultFormula.InnerText.Trim())"
}
...
$selection = Require-SingleNode -XPath "/dxl:database/dxl:view[@name='XPScriptTestView']/dxl:code[@event='selection']/dxl:formula" -Label 'XPScriptTestView selection formula'
if ($selection.InnerText.Trim() -ne 'Form = "XPScriptTest"') {
    throw "XPScriptTestView selection formula changed unexpectedly: $($selection.InnerText.Trim())"
}
...
if ($subjectColumn.GetAttribute('sort') -ne 'ascending') {
    throw 'XPScriptTestView Subject column must remain ascending for deterministic key tests.'
}
...
if ($agentScript.InnerText -notmatch 'XPSCRIPT-AGENT=OK') {
    throw 'XPScriptTestAgent is missing the no-context success marker.'
}
if ($agentScript.InnerText -notmatch 'XPSCRIPT-AGENT-DOC=') {
    throw 'XPScriptTestAgent is missing the document-context marker.'
}

$expectedDocuments = @(
    @{ Subject = 'Alpha'; Marker = 'fixture-alpha'; TextList = @('one', 'two', 'three'); Number = '12.5' },
    @{ Subject = 'Beta'; Marker = 'fixture-beta'; TextList = @('red', 'green', 'blue'); Number = '99' }
)
...
foreach ($expected in $expectedDocuments) {
    ...
    if ($null -eq $markerNode -or $markerNode.InnerText -ne $expected.Marker) {
        throw "Fixture document $subject has the wrong XPScriptMarker."
    }

    $numberNode = $document.SelectSingleNode("dxl:item[@name='NumberValue']/dxl:number", $ns)
    if ($null -eq $numberNode -or $numberNode.InnerText -ne $expected.Number) {
        throw "Fixture document $subject has the wrong NumberValue."
    }
    ...
    if ($actualTextList.Count -ne $expected.TextList.Count) {
        throw "Fixture document $subject has the wrong TextList length."
    }
    for ($i = 0; $i -lt $expected.TextList.Count; $i++) {
        if ($actualTextList[$i] -ne $expected.TextList[$i]) {
            throw "Fixture document $subject has an unexpected TextList value at index $i."
        }
    }

    $richText = $document.SelectSingleNode("dxl:item[@name='Body']/dxl:richtext", $ns)
    if ($null -eq $richText) {
        throw "Fixture document $subject must contain a rich-text Body item."
    }
}

$fixtureDocuments = @($dxl.SelectNodes("/dxl:database/dxl:document[@form='XPScriptTest']", $ns))
if ($fixtureDocuments.Count -ne $expectedDocuments.Count) {
    throw "Expected exactly $($expectedDocuments.Count) XPScriptTest fixture documents, found $($fixtureDocuments.Count)."
}
