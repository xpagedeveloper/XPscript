...
async Task<long> RunAsync(bool debug)
{
...
Console.WriteLine($"runColdMs={cold} runWarmMs={string.Join(",",warmSamples)} runWarmMedianMs={Median(warmSamples)} debugFreshMs={string.Join(",",debugSamples)} debugFreshMedianMs={Median(debugSamples)}");
return 0;
