...
var validation=await CallAsync(new {jsonrpc="2.0",id=3,method="tools/call",@params=new{name="xpscript_validate",arguments=new{source="Sub Main()\\nDim value As Integer\\nvalue = Nothing\\nEnd Sub\\n",filename="agent.xps"}}});
...
if (!structured.GetProperty("errors").EnumerateArray().Any(x=>x.GetProperty("diagnosticCode").GetString()=="XPS2001")) throw new Exception("Missing XPS2001 diagnostic.");
