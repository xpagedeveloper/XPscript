...
pre = Path('src/XPScript.Compiler/SourceLineContinuationPreprocessor.cs')
pre.write_text(r'''namespace XPScript.Compiler;
...
needle = '''        source = new IfLayoutPreprocessor().Transform(source);\n        source = new SourceLineMarkerPreprocessor().Transform(source);\n'''
replacement = '''        source = new IfLayoutPreprocessor().Transform(source);\n        source = new SourceLineContinuationPreprocessor().Transform(source);\n        source = new SourceLineMarkerPreprocessor().Transform(source);\n'''
if replacement not in text:
    if needle not in text: raise SystemExit('Pipeline insertion point not found')
